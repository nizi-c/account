# 设计文档

## 概述

收支记账APP是一个基于HarmonyOS的移动应用，使用ArkTS语言开发。该应用采用MVVM架构模式，通过关系型数据库进行数据持久化，提供直观的用户界面用于记录和分析个人收支情况。应用支持情绪记账、成就系统等创新功能，并通过响应式设计适配手机和平板设备。

## 架构

### 整体架构

应用采用分层架构设计：

```
┌─────────────────────────────────────┐
│         Presentation Layer          │
│    (Pages, Components, ViewModels)  │
├─────────────────────────────────────┤
│         Business Logic Layer        │
│    (Services, Managers, Utils)      │
├─────────────────────────────────────┤
│         Data Access Layer           │
│    (Repositories, Models, DAO)      │
├─────────────────────────────────────┤
│         Storage Layer               │
│    (RelationalStore, Preferences)   │
└─────────────────────────────────────┘
```

### 技术栈

- **UI框架**: ArkUI (声明式UI)
- **语言**: ArkTS (TypeScript扩展)
- **数据库**: RelationalStore (关系型数据库)
- **状态管理**: @State, @Prop, @Link装饰器
- **图表库**: 自定义Canvas绘制或第三方图表组件
- **本地存储**: Preferences (用户偏好设置)

## 组件和接口

### 核心组件

#### 1. 数据模型层

**Transaction (交易记录模型)**
```typescript
interface Transaction {
  id: number;                    // 主键
  type: TransactionType;         // 收入/支出
  amount: number;                // 金额
  category: string;              // 分类
  description: string;           // 用途描述
  date: number;                  // 时间戳
  mood?: MoodType;               // 情绪标签（可选）
  createdAt: number;             // 创建时间
  updatedAt: number;             // 更新时间
}

enum TransactionType {
  INCOME = 'income',
  EXPENSE = 'expense'
}

enum MoodType {
  SATISFIED = 'satisfied',      // 😊满意
  REGRET = 'regret',            // 😟后悔
  SURPRISE = 'surprise'         // 🎉惊喜
}
```

**Category (分类模型)**
```typescript
interface Category {
  id: string;
  name: string;
  type: TransactionType;
  icon: string;                  // 图标资源路径
  color: string;                 // 分类颜色
}
```

**Achievement (成就模型)**
```typescript
interface Achievement {
  id: string;
  name: string;
  description: string;
  icon: string;
  unlocked: boolean;
  unlockedAt?: number;
  progress: number;              // 0-100
  target: number;
}
```

**Budget (预算模型)**
```typescript
interface Budget {
  id: number;
  type: BudgetType;              // 月度/周度/分类
  period: string;                // 时间段标识（如"2024-01"或"2024-W01"）
  category?: string;             // 分类预算的分类名称（可选）
  amount: number;                // 预算金额
  spent: number;                 // 已花费金额
  remaining: number;             // 剩余金额
  percentage: number;            // 使用百分比
  status: BudgetStatus;          // 正常/预警/超支
  createdAt: number;
  updatedAt: number;
}

enum BudgetType {
  MONTHLY = 'monthly',
  WEEKLY = 'weekly',
  CATEGORY = 'category'
}

enum BudgetStatus {
  NORMAL = 'normal',             // < 80%
  WARNING = 'warning',           // 80% - 100%
  EXCEEDED = 'exceeded'          // > 100%
}
```

**ExportData (导出数据模型)**
```typescript
interface ExportData {
  version: string;               // 数据版本
  exportDate: number;            // 导出时间
  transactions: Transaction[];
  categories: Category[];
  achievements: Achievement[];
  budgets?: Budget[];
}
```

**Summary (汇总统计模型)**
```typescript
interface Summary {
  period: PeriodType;
  startDate: number;
  endDate: number;
  totalIncome: number;
  totalExpense: number;
  netAmount: number;
  categoryBreakdown: CategorySummary[];
}

interface CategorySummary {
  category: string;
  amount: number;
  percentage: number;
  count: number;
}

enum PeriodType {
  DAY = 'day',
  WEEK = 'week',
  MONTH = 'month',
  YEAR = 'year'
}
```

#### 2. 数据访问层

**TransactionRepository**
```typescript
class TransactionRepository {
  // 创建交易记录
  async create(transaction: Omit<Transaction, 'id'>): Promise<Transaction>
  
  // 删除交易记录
  async delete(id: number): Promise<boolean>
  
  // 查询交易记录
  async query(filter: TransactionFilter): Promise<Transaction[]>
  
  // 获取所有交易记录
  async getAll(): Promise<Transaction[]>
  
  // 按日期范围查询
  async getByDateRange(startDate: number, endDate: number): Promise<Transaction[]>
  
  // 按分类查询
  async getByCategory(category: string): Promise<Transaction[]>
}

interface TransactionFilter {
  type?: TransactionType;
  categories?: string[];
  minAmount?: number;
  maxAmount?: number;
  startDate?: number;
  endDate?: number;
  description?: string;
  mood?: MoodType;
}
```

**CategoryRepository**
```typescript
class CategoryRepository {
  // 获取所有分类
  async getAll(): Promise<Category[]>
  
  // 按类型获取分类
  async getByType(type: TransactionType): Promise<Category[]>
  
  // 获取预定义分类
  getDefaultCategories(): Category[]
}
```

**AchievementRepository**
```typescript
class AchievementRepository {
  // 获取所有成就
  async getAll(): Promise<Achievement[]>
  
  // 更新成就状态
  async updateAchievement(id: string, unlocked: boolean, progress: number): Promise<void>
  
  // 获取已解锁成就
  async getUnlocked(): Promise<Achievement[]>
}
```

**BudgetRepository**
```typescript
class BudgetRepository {
  // 创建预算
  async create(budget: Omit<Budget, 'id'>): Promise<Budget>
  
  // 更新预算
  async update(id: number, budget: Partial<Budget>): Promise<Budget>
  
  // 删除预算
  async delete(id: number): Promise<boolean>
  
  // 获取月度预算
  async getMonthlyBudget(period: string): Promise<Budget | null>
  
  // 获取周度预算
  async getWeeklyBudget(period: string): Promise<Budget | null>
  
  // 获取分类预算
  async getCategoryBudgets(period: string): Promise<Budget[]>
  
  // 获取所有预算
  async getAll(): Promise<Budget[]>
}
```

#### 3. 业务逻辑层

**TransactionService**
```typescript
class TransactionService {
  // 添加交易
  async addTransaction(data: TransactionInput): Promise<Transaction>
  
  // 删除交易
  async deleteTransaction(id: number): Promise<void>
  
  // 查询交易
  async searchTransactions(filter: TransactionFilter): Promise<Transaction[]>
  
  // 获取日汇总
  async getDailySummary(date: number): Promise<Summary>
  
  // 获取月汇总
  async getMonthlySummary(year: number, month: number): Promise<Summary>
  
  // 获取周汇总
  async getWeeklySummary(date: number): Promise<Summary>
  
  // 获取年汇总
  async getYearlySummary(year: number): Promise<Summary>
}
```

**AchievementService**
```typescript
class AchievementService {
  // 检查并更新成就
  async checkAchievements(): Promise<Achievement[]>
  
  // 检查连续记账成就
  async checkConsecutiveDays(): Promise<void>
  
  // 检查预算控制成就
  async checkBudgetControl(): Promise<void>
  
  // 检查消费合理化成就
  async checkReasonableSpending(): Promise<void>
  
  // 获取成就进度
  async getAchievementProgress(id: string): Promise<number>
}
```

**StatisticsService**
```typescript
class StatisticsService {
  // 生成饼图数据
  async generatePieChartData(period: PeriodType, date: number): Promise<ChartData>
  
  // 生成折线图数据
  async generateLineChartData(startDate: number, endDate: number): Promise<ChartData>
  
  // 生成情绪日历数据
  async generateMoodCalendar(year: number, month: number): Promise<MoodCalendarData>
  
  // 计算分类占比
  async calculateCategoryPercentage(period: PeriodType, date: number): Promise<CategorySummary[]>
}
```

**BudgetService**
```typescript
class BudgetService {
  // 设置月度预算
  async setMonthlyBudget(year: number, month: number, amount: number): Promise<Budget>
  
  // 设置周度预算
  async setWeeklyBudget(year: number, week: number, amount: number): Promise<Budget>
  
  // 设置分类预算
  async setCategoryBudget(period: string, category: string, amount: number): Promise<Budget>
  
  // 获取预算状态
  async getBudgetStatus(budgetId: number): Promise<BudgetStatus>
  
  // 更新预算进度
  async updateBudgetProgress(period: string): Promise<void>
  
  // 检查预算预警
  async checkBudgetWarnings(): Promise<Budget[]>
  
  // 获取当前月度预算
  async getCurrentMonthlyBudget(): Promise<Budget | null>
  
  // 获取当前周度预算
  async getCurrentWeeklyBudget(): Promise<Budget | null>
  
  // 获取所有分类预算
  async getCategoryBudgets(period: string): Promise<Budget[]>
}
```

**ExportService**
```typescript
class ExportService {
  // 导出为CSV格式
  async exportToCSV(startDate?: number, endDate?: number): Promise<string>
  
  // 导出为JSON格式
  async exportToJSON(startDate?: number, endDate?: number): Promise<ExportData>
  
  // 保存导出文件
  async saveExportFile(content: string, filename: string, format: 'csv' | 'json'): Promise<string>
  
  // 生成CSV内容
  generateCSVContent(transactions: Transaction[]): string
  
  // 生成完整备份数据
  async generateFullBackup(): Promise<ExportData>
}
```

#### 4. 表现层

**页面组件**

- **HomePage**: 首页，显示当日收支概览和快速添加入口
- **AddTransactionPage**: 添加交易页面，包含表单输入
- **TransactionListPage**: 交易列表页面，支持侧滑删除
- **StatisticsPage**: 统计页面，显示图表和汇总数据
- **QueryPage**: 查询页面，提供高级筛选功能
- **MoodCalendarPage**: 情绪日历页面
- **AchievementPage**: 成就页面
- **BudgetPage**: 预算管理页面，显示预算设置和进度
- **ExportPage**: 数据导出页面，提供导出选项

**可复用组件**

- **TransactionCard**: 交易记录卡片
- **CategorySelector**: 分类选择器
- **MoodSelector**: 情绪选择器
- **DateRangePicker**: 日期范围选择器
- **PieChart**: 饼图组件
- **LineChart**: 折线图组件
- **SummaryCard**: 汇总卡片
- **AchievementBadge**: 成就徽章
- **BudgetProgressBar**: 预算进度条组件
- **BudgetCard**: 预算卡片组件
- **WarningAlert**: 预警提示组件

## 数据模型

### 数据库设计

**transactions 表**
```sql
CREATE TABLE transactions (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  type TEXT NOT NULL,
  amount REAL NOT NULL,
  category TEXT NOT NULL,
  description TEXT,
  date INTEGER NOT NULL,
  mood TEXT,
  created_at INTEGER NOT NULL,
  updated_at INTEGER NOT NULL
)
```

**achievements 表**
```sql
CREATE TABLE achievements (
  id TEXT PRIMARY KEY,
  name TEXT NOT NULL,
  description TEXT,
  icon TEXT,
  unlocked INTEGER DEFAULT 0,
  unlocked_at INTEGER,
  progress REAL DEFAULT 0,
  target REAL NOT NULL
)
```

**categories 表**
```sql
CREATE TABLE categories (
  id TEXT PRIMARY KEY,
  name TEXT NOT NULL,
  type TEXT NOT NULL,
  icon TEXT,
  color TEXT
)
```

**budgets 表**
```sql
CREATE TABLE budgets (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  type TEXT NOT NULL,
  period TEXT NOT NULL,
  category TEXT,
  amount REAL NOT NULL,
  spent REAL DEFAULT 0,
  remaining REAL NOT NULL,
  percentage REAL DEFAULT 0,
  status TEXT DEFAULT 'normal',
  created_at INTEGER NOT NULL,
  updated_at INTEGER NOT NULL,
  UNIQUE(type, period, category)
)
```

### 数据关系

- Transaction 与 Category 是多对一关系（一个分类可以有多个交易）
- Transaction 与 MoodType 是可选的一对一关系
- Achievement 独立存储，通过业务逻辑与 Transaction 关联
- Budget 与时间段和分类关联，通过业务逻辑计算支出进度
- Budget 与 Transaction 通过时间段和分类间接关联

## 正确性属性

*属性是一个特征或行为，应该在系统的所有有效执行中保持为真——本质上是关于系统应该做什么的正式声明。属性作为人类可读规范和机器可验证正确性保证之间的桥梁。*


### 属性反思

在编写具体属性之前，我们需要识别和消除冗余：

**识别的冗余：**
- 属性 3.1（当日交易过滤）和 3.3（当月交易过滤）可以合并为一个通用的日期范围过滤属性
- 属性 3.2（当日汇总）、3.4（当月汇总）、4.1（周汇总）、4.2（月汇总）、4.3（年汇总）都是汇总计算，可以合并为一个通用的汇总正确性属性
- 属性 2.2（收入分类过滤）和 2.3（支出分类过滤）可以合并为一个按类型过滤分类的属性
- 属性 5.1-5.4（各种查询条件）都是查询过滤的不同维度，可以用一个综合的查询正确性属性覆盖

**保留的独特属性：**
- 数据持久化往返属性（11.1, 11.2）
- 删除操作的正确性（1.3）
- 输入验证（1.4）
- 排序正确性（3.5）
- 占比计算正确性（4.5）
- 成就解锁逻辑（8.1, 8.2, 8.3）
- 情绪关联和过滤（7.2, 7.4, 7.5）

### 核心属性

**属性 1: 交易持久化往返一致性**
*对于任何*有效的交易数据，创建并保存后再从存储中加载，应该得到等价的交易记录（除了系统生成的ID和时间戳）
**验证需求：11.1, 11.2**

**属性 2: 删除操作的完整性**
*对于任何*存在的交易记录，执行删除操作后，该记录不应该出现在任何查询结果中，且相关汇总数据应该更新
**验证需求：1.3, 1.5**

**属性 3: 输入验证的严格性**
*对于任何*无效的交易输入（负数金额、零金额、无效日期格式），系统应该拒绝创建交易记录
**验证需求：1.4**

**属性 4: 日期范围查询的准确性**
*对于任何*日期范围查询，返回的所有交易记录的日期都应该在指定范围内（包含边界）
**验证需求：3.1, 3.3, 5.3**

**属性 5: 汇总计算的正确性**
*对于任何*时间段（日/周/月/年）的交易集合，汇总的收入总额应该等于所有收入交易的金额之和，支出总额应该等于所有支出交易的金额之和，净收支应该等于收入减去支出
**验证需求：3.2, 3.4, 4.1, 4.2, 4.3**

**属性 6: 时间倒序排列的一致性**
*对于任何*交易记录列表，排序后的列表应该满足：对于任意相邻的两条记录，前一条的日期应该大于或等于后一条的日期
**验证需求：3.5**

**属性 7: 分类占比计算的完整性**
*对于任何*时间段的汇总数据，所有分类的占比之和应该等于100%（误差在0.01%以内）
**验证需求：4.5**

**属性 8: 分类过滤的类型一致性**
*对于任何*交易类型（收入/支出），获取的分类列表中所有分类的类型都应该与请求的类型匹配
**验证需求：2.2, 2.3**

**属性 9: 分类汇总的准确性**
*对于任何*交易集合，按分类汇总后，每个分类的金额应该等于该分类下所有交易金额之和
**验证需求：2.4**

**属性 10: 金额范围查询的边界正确性**
*对于任何*金额范围查询（最小值、最大值），返回的所有交易金额都应该在指定范围内（包含边界）
**验证需求：5.1**

**属性 11: 关键词搜索的包含性**
*对于任何*用途关键词搜索，返回的所有交易的描述字段都应该包含该关键词（不区分大小写）
**验证需求：5.2**

**属性 12: 分类过滤的精确性**
*对于任何*指定的分类，查询返回的所有交易都应该属于该分类
**验证需求：5.4**

**属性 13: 复合查询的交集正确性**
*对于任何*多个查询条件的组合，返回的交易记录应该同时满足所有指定的条件
**验证需求：5.5**

**属性 14: 情绪标签关联的持久性**
*对于任何*带有情绪标签的交易，保存后再查询，情绪标签应该保持一致
**验证需求：7.2**

**属性 15: 情绪过滤的准确性**
*对于任何*指定日期的情绪交易查询，返回的所有交易都应该是该日期且包含情绪标签的记录
**验证需求：7.5**

**属性 16: 连续记账成就的解锁条件**
*对于任何*用户的交易历史，如果存在连续7天都有交易记录，则"连续记账7天"成就应该被解锁
**验证需求：8.1**

**属性 17: 预算控制成就的解锁条件**
*对于任何*月份，如果该月的总支出小于或等于预设预算，则"月度预算控制达人"成就应该被解锁
**验证需求：8.2**

**属性 18: 餐饮消费成就的解锁条件**
*对于任何*时间段，如果餐饮分类的支出占总支出的比例低于设定阈值（如30%），则"餐饮消费合理化"成就应该被解锁
**验证需求：8.3**

**属性 19: 数据迁移的向后兼容性**
*对于任何*旧版本的数据结构，执行迁移后，所有原有的交易记录都应该能被正确读取且数据完整
**验证需求：11.5**

**属性 20: 预算进度计算的准确性**
*对于任何*预算和对应时间段的交易集合，预算的已花费金额应该等于该时间段内所有支出交易的金额之和，剩余金额应该等于预算金额减去已花费金额
**验证需求：12.4, 12.7**

**属性 21: 预算状态判断的正确性**
*对于任何*预算，当使用百分比小于80%时状态应为正常，80%-100%时应为预警，大于100%时应为超支
**验证需求：12.5, 12.6**

**属性 22: 分类预算过滤的准确性**
*对于任何*分类预算，计算已花费金额时应该只包含该分类的支出交易
**验证需求：12.3, 12.8**

**属性 23: CSV导出的完整性**
*对于任何*交易记录集合，导出为CSV后再解析，应该能恢复所有关键字段（日期、类型、分类、金额、用途）
**验证需求：13.2, 13.6**

**属性 24: JSON导出的往返一致性**
*对于任何*导出的JSON数据，应该包含所有交易记录、分类定义和成就数据，且数据结构完整
**验证需求：13.3, 13.8**

**属性 25: 日期范围导出的准确性**
*对于任何*指定的日期范围，导出的交易记录都应该在该日期范围内
**验证需求：13.5**

## 错误处理

### 错误类型

1. **输入验证错误**
   - 金额为负数或零
   - 日期格式无效
   - 必填字段缺失
   - 处理：显示具体的错误提示，保留用户输入

2. **数据库操作错误**
   - 连接失败
   - 写入失败
   - 读取失败
   - 处理：显示友好的错误信息，提供重试选项，记录错误日志

3. **数据不一致错误**
   - 引用的分类不存在
   - 交易记录损坏
   - 处理：尝试自动修复，无法修复时提示用户

4. **网络错误**（如果有云同步功能）
   - 连接超时
   - 同步失败
   - 处理：使用本地缓存，后台重试

### 错误恢复策略

- **事务性操作**：使用数据库事务确保数据一致性
- **数据备份**：定期自动备份用户数据
- **降级处理**：关键功能失败时提供基础功能
- **用户反馈**：所有错误都应该有清晰的用户提示

## 测试策略

### 双重测试方法

本项目采用单元测试和基于属性的测试相结合的方法：

- **单元测试**：验证特定示例、边缘情况和错误条件
- **基于属性的测试**：验证应该在所有输入中保持的通用属性
- 两者互补：单元测试捕获具体的bug，基于属性的测试验证通用正确性

### 单元测试

**测试范围：**
- 数据模型的创建和验证
- Repository的CRUD操作
- Service层的业务逻辑
- UI组件的渲染和交互
- 边缘情况（空列表、边界值、特殊字符）

**测试工具：**
- HarmonyOS测试框架（@ohos/hypium）
- Mock工具（@ohos/hamock）

**示例测试场景：**
- 创建交易时所有字段都正确保存
- 删除不存在的交易返回适当错误
- 空交易列表显示空状态UI
- 日期边界情况（月初、月末、闰年）

### 基于属性的测试

**测试库：** 使用fast-check（TypeScript的属性测试库）或自定义实现

**配置：** 每个属性测试至少运行100次迭代

**标注格式：** 每个属性测试必须使用注释标注对应的设计文档属性
```typescript
// **Feature: expense-tracker, Property 1: 交易持久化往返一致性**
```

**测试策略：**

1. **数据生成器**
   - 随机交易生成器（有效和无效数据）
   - 随机日期范围生成器
   - 随机查询条件生成器
   - 随机分类生成器

2. **属性测试覆盖**
   - 每个正确性属性对应一个属性测试
   - 测试应该覆盖各种输入组合
   - 使用收缩（shrinking）找到最小失败案例

3. **测试示例**

```typescript
// **Feature: expense-tracker, Property 1: 交易持久化往返一致性**
test('property: transaction persistence round-trip', async () => {
  await fc.assert(
    fc.asyncProperty(
      transactionArbitrary(), // 随机交易生成器
      async (transaction) => {
        const saved = await repository.create(transaction);
        const loaded = await repository.getById(saved.id);
        
        // 验证往返一致性（忽略系统生成字段）
        expect(loaded.amount).toBe(transaction.amount);
        expect(loaded.category).toBe(transaction.category);
        expect(loaded.description).toBe(transaction.description);
        // ... 其他字段
      }
    ),
    { numRuns: 100 }
  );
});

// **Feature: expense-tracker, Property 5: 汇总计算的正确性**
test('property: summary calculation correctness', async () => {
  await fc.assert(
    fc.asyncProperty(
      fc.array(transactionArbitrary()), // 随机交易数组
      periodArbitrary(), // 随机时间段
      async (transactions, period) => {
        // 保存所有交易
        for (const t of transactions) {
          await repository.create(t);
        }
        
        const summary = await service.getSummary(period);
        
        // 手动计算期望值
        const expectedIncome = transactions
          .filter(t => t.type === 'income' && isInPeriod(t.date, period))
          .reduce((sum, t) => sum + t.amount, 0);
        
        const expectedExpense = transactions
          .filter(t => t.type === 'expense' && isInPeriod(t.date, period))
          .reduce((sum, t) => sum + t.amount, 0);
        
        // 验证汇总正确性
        expect(summary.totalIncome).toBeCloseTo(expectedIncome, 2);
        expect(summary.totalExpense).toBeCloseTo(expectedExpense, 2);
        expect(summary.netAmount).toBeCloseTo(expectedIncome - expectedExpense, 2);
      }
    ),
    { numRuns: 100 }
  );
});
```

### 集成测试

- 测试完整的用户流程（添加交易 → 查看列表 → 查询 → 删除）
- 测试数据库迁移场景
- 测试不同屏幕尺寸的响应式布局

### UI测试

- 组件渲染测试
- 用户交互测试（点击、滑动、输入）
- 响应式布局测试
- 无障碍性测试

## 性能考虑

### 优化策略

1. **数据库优化**
   - 为常用查询字段（date, category, type）创建索引
   - 使用批量操作减少数据库访问次数
   - 实现分页加载大量交易记录

2. **UI性能**
   - 使用虚拟滚动处理长列表
   - 图表数据采样（大数据集时）
   - 懒加载非关键组件
   - 使用@State优化状态更新

3. **计算优化**
   - 缓存汇总计算结果
   - 增量更新统计数据
   - 使用Web Worker处理复杂计算（如果支持）

### 性能目标

- 应用启动时间：< 2秒
- 交易列表加载：< 500ms（100条记录）
- 汇总计算：< 500ms
- UI交互响应：< 100ms
- 数据库操作：< 200ms

## 安全性

### 数据安全

- 所有数据存储在设备本地
- 使用HarmonyOS的安全存储API
- 敏感数据（如果有）使用加密存储

### 输入安全

- 所有用户输入进行验证和清理
- 防止SQL注入（使用参数化查询）
- 限制输入长度防止溢出

## 可访问性

- 支持屏幕阅读器
- 提供足够的颜色对比度
- 可触摸目标至少48dp
- 支持字体缩放
- 提供替代文本

## 国际化

- 支持多语言（中文、英文）
- 货币格式本地化
- 日期格式本地化
- 数字格式本地化

## 未来扩展

### 可能的功能

- 云同步和备份
- 预算管理和提醒
- 账单扫描和OCR识别
- 多账户支持
- 数据导出（CSV, Excel）
- 更多图表类型
- 自定义分类
- 定期交易模板
- 财务报告生成

### 架构扩展性

- 模块化设计便于添加新功能
- 使用接口抽象便于替换实现
- 插件化的成就系统
- 可配置的图表引擎
