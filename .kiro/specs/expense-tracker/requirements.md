...
**用户故事：** 作为用户，我想要设置和管理预算目标，以便控制我的支出并避免超支。

#### 验收标准

1. WHEN 用户设置月度预算目标 THEN 系统 SHALL 保存该预算金额并关联到指定月份
2. WHEN 用户设置周度预算目标 THEN 系统 SHALL 保存该预算金额并关联到指定周
3. WHEN 用户设置分类预算 THEN 系统 SHALL 保存该分类的预算金额（如餐饮、交通等）
4. WHEN 用户查看预算页面 THEN 系统 SHALL 显示实时预算进度条，展示已用金额和剩余金额
5. WHEN 当前支出达到预算的80% THEN 系统 SHALL 显示预警提醒
6. WHEN 当前支出超过预算 THEN 系统 SHALL 显示超支警告并标红显示超支金额
7. WHEN 用户添加新交易 THEN 系统 SHALL 立即更新相关预算的进度
8. WHEN 用户查看分类预算 THEN 系统 SHALL 显示每个分类的预算使用情况和占比

### 需求 13：数据导出功能

**用户故事：** 作为用户，我想要导出我的交易数据，以便进行备份或在其他应用中使用。

#### 验收标准

1. WHEN 用户选择导出数据功能 THEN 系统 SHALL 提供CSV和JSON两种导出格式选项
2. WHEN 用户选择CSV格式导出 THEN 系统 SHALL 生成包含所有交易记录的CSV文件
3. WHEN 用户选择JSON格式导出 THEN 系统 SHALL 生成包含所有交易记录和元数据的JSON文件
4. WHEN 导出文件生成完成 THEN 系统 SHALL 将文件保存到用户可访问的目录并显示成功提示
5. WHEN 用户选择日期范围导出 THEN 系统 SHALL 仅导出指定日期范围内的交易记录
6. WHEN 导出的CSV文件打开时 THEN 系统 SHALL 确保文件包含表头（日期、类型、分类、金额、用途、情绪）
7. WHEN 导出失败 THEN 系统 SHALL 显示错误信息并提供重试选项
8. WHEN 用户导出数据 THEN 系统 SHALL 同时导出分类定义和成就数据以支持完整备份
